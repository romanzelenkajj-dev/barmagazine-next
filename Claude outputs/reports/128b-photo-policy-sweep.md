# Task 128b: photo policy applied, sweep of every photo Code placed since 20 September

Date: 2026-09-24. Revised the same day after Roman's clarification: licensed photos stay.

## The rule as it now stands (in SETUP.md)

Code may take one photo from a bar's own website or Instagram, interiors only (the room, or the counter with its seating, as a guest sees it), never from Falstaff, Google, press or third-party sites, credited "Photo: <Bar name>" inside the hero. Licensed sources are a separate thing and are allowed: owner uploads, PR-agency photos, and The 50 Best Bars media pack used with the agency's permission and credited "Photo courtesy of The 50 Best Bars". What the rule forbids is Code taking an image from a third-party site on its own initiative. A licensed photo is never removed for failing the own-site test. Before publishing anything it scrapes, Code stages candidates in `outreach/photos-staged/<slug>/`, cropped to the hero ratio, and lists them as a contact sheet for approval.

## 1. Heidelberg (task 128)

- **Schilling Roofbar:** the bottle-shelf photo was replaced with the dining-room frame from the bar's own site before the approval step arrived. Live, excluded from the sheet at Roman's instruction.
- **Bent Bar, 15 High, Cocktail-Café Regie:** live and on the contact sheet awaiting approval.
- **Frollein Bent, Lino's Bar:** no photo. Nothing on their own channels passes (the shared bentbar.de site has no garden image, Lino's has no site).
- All four live heroes read "Photo: <Bar name>".

## 2. Every photo Code uploaded since 20 September

30 photo files. Four are owner uploads through the dashboard: KAA° Mixology, Cantina OK!, Bar Martinelli, and Otro Bar's row is owner-claimed. The other 23 are the task 113 photos of 23 September from The 50 Best Bars media pack, credited "courtesy of The 50 Best Bars".

**Nothing is removed and nothing is swapped.** The pack is licensed, so all 23 stay exactly as they are (Roman, 2026-09-24). An earlier version of this report proposed removing them, and a later one proposed swapping Boadas for an own-site shot; both are withdrawn. No photo was deleted from any of the 23 profiles, and the own-site candidates gathered for the comparison have been deleted from staging.

## 3. Heidelberg, approved

Roman approved 15-high-heidelberg, cocktail-cafe-regie-heidelberg and schilling-roofbar-heidelberg on 24 September; all three were already live and stay, credited "Photo: <Bar name>". Their staged copies are deleted.

Bent Bar was rejected: the photo is a tall portrait and the hero crops it to its middle band, which is mostly chair legs and floor. Two wider interiors exist in the bar's own media library at bentbar.de, neither linked from its home page, and both are staged in `outreach/photos-staged/bent-bar-heidelberg/` with the sheet at `outreach/photos-staged/bent-bar-2026-09-24.html`. The portrait photo stays live until Roman picks, since the instruction was to stage and not publish.

## 4. Waiting on Roman

"bent 01", "bent 02" or "bent none".
