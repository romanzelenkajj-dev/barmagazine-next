...
// Addresses that asked not to be emailed again (privacy policy: "If you opt
// out, we will not email that address again"). One address per line in
// outreach/optout.txt, '#' comments allowed. No bypass flag, same as the
// corporate list - an opt-out is an opt-out.
const OPTOUT_PATH = new URL('../outreach/optout.txt', import.meta.url).pathname;
function loadOptOuts() {
  try {
    return new Set(
      readFileSync(OPTOUT_PATH, 'utf8')
        .split('\n')
...
const OPTED_OUT = loadOptOuts();

// Bars parked from outreach by editorial decision (chain inboxes, PR
// agencies, operators in the relationship parking lot). Slug-keyed, in
// outreach/parked.txt with the reason beside each. Skipped before the email
// is even looked at, so a parked bar cannot slip through on a later address.
const PARKED_PATH = new URL('../outreach/parked.txt', import.meta.url).pathname;
function loadParked() {
  try {
    return new Set(
      readFileSync(PARKED_PATH, 'utf8')
        .split('\n')
...
function textFor(bar) {
  return [
    ...
    "One thing most listings are still missing is a photo. A listing with a photo is ranked above one without it on our city pages, and adding yours takes a minute once the bar is claimed. Reply to this email with your favorite shot of the bar and we'll add it to your profile, free.", '',
