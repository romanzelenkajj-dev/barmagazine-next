...
/** Working share bar: native share on mobile, platform links + copy. */
export function ShareBar({ url, title }: { url: string; title: string }) {
  ...
  return (
    <div className="share-bar">
      <span>Share</span>
      <button className="share-btn" title="Share on X" aria-label="Share on X"
      ...
      <button className="share-btn" title="WhatsApp" aria-label="Share on WhatsApp"
        onClick={async () => { if (!(await nativeShare())) open(`https://wa.me/?text=${enc(`${title} ${url}`)}`); }}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
      ...
      <button className="share-btn" title="Copy link" aria-label="Copy link" onClick={copy}>
        {copied ? (
