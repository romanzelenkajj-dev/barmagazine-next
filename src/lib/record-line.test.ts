...
import { recordLine, recordSummary } from './record-line';
...
describe('recordLine', () => {
  it('names no bar and lists only non-zero programs', () => {
    const bars = [
  ...
  it('gives no line when nothing is on the record', () => {
    expect(recordLine([{}, {}, {}, {}, {}], 'speakeasies')).toBeNull();
    expect(recordSummary([{ accolades: [] }])).toBeNull();
  });

  it('stops at the two highest-ranked programs, winners over nominees inside one', () => {
    const bars = [
      { accolades: [totc('winner'), totc('nominee'), totc('nominee')] },
      { accolades: [totc('nominee'), jbf('winner'), pin()] },
      { accolades: [a50b(5), a50b(9), jbf('nominee')] },
    ];
    expect(recordLine(bars)).toBe("Three bars, chosen on the record: one on Asia's 50 Best, one Spirited Awards winner.");
    expect(recordSummary([{ accolades: [jbf('nominee'), pin()] }, { accolades: [pin()] }]))
      .toBe('one James Beard nominee, two with Pinnacle Guide pins');
  });
  ...
  it('uses digits past ten and the singular noun for one', () => {
    ...
    expect(recordLine([{ accolades: [totc('nominee')] }], 'pubs')).toBe('One pub, chosen on the record: one Spirited Awards nominee.');
  });
});
