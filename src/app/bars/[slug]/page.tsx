...
export default async function BarProfilePage({ params }: { params: { slug: string } }) {
  ...
  // Use 2-column layout when there's enough main content (description or photo gallery)
  // Otherwise use a cleaner single-column layout
          ...
          <aside className="bar-profile-sidebar">
                ...
                <ul className="bar-profile-info-list">
                  {bar.website && (
                    <li>
                      ...
                      <a href={bar.website} target="_blank" rel="noopener noreferrer">{bar.website.replace(/^https?:\/\/(www\.)?/, '').replace(/\/$/, '')}</a>
                    </li>
                  )}
                  {bar.instagram && (
                    <li>
                      ...
                      <a href={`https://instagram.com/${bar.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer">@{bar.instagram.replace('@', '')}</a>
                    </li>
                  )}
                  {bar.phone && (
                    <li>
            ...
            {bar.address && (
              <div className="bar-profile-info-card">
                ...
                {bar.lat && bar.lng && (
                  <a href={`https://www.google.com/maps?q=${bar.lat},${bar.lng}`} target="_blank" rel="noopener noreferrer" className="bar-profile-map-link">
                    Open in Google Maps
            ...
            {bar.tier === 'free' && (
              <div className="bar-profile-upgrade">
                ...
                <Link href="/claim-your-bar">Get Listed</Link>
              </div>
            )}
          </aside>
        </div>
      ) : (
        /* Single-column layout for bars with only article + contact info */
        <div className="bar-profile-single">
          {bar.wp_article_slug && (
            <Link href={`/${bar.wp_article_slug}`} className="bar-profile-feature-card">
              <div className="bar-profile-feature-card-icon">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M4 19.5A2.5 2.5 0 016.5 17H20" />
                  <path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z" />
                </svg>
              </div>
              <div className="bar-profile-feature-card-content">
                <span className="bar-profile-feature-card-label">BarMagazine Feature</span>
                <span className="bar-profile-feature-card-title">Read the full article about {bar.name}</span>
              </div>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="bar-profile-feature-card-arrow">
                <path d="M5 12h14M12 5l7 7-7 7" />
              </svg>
            </Link>
          )}

          {(hasContact || bar.address) && (
            <div className="bar-profile-details-grid">
              {hasContact && (
                <div className="bar-profile-info-card">
                  <h3>Contact</h3>
                  <ul className="bar-profile-info-list">
