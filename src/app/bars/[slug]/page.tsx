...
export default async function BarProfilePage({ params }: { params: { slug: string } }) {
  ...
  // The lead badge shows the most SPECIFIC style from the type union, not
  // the raw type column: nearly everything is a cocktail bar, so a
  // Speakeasy-tagged bar leads with Speakeasy. The remaining union members
  // render as secondary tags, each linking to its type-city guide when the
        ...
        <div className="bar-v2-info">
            ...
            <div className="bar-v2-actions">
              ...
              {/* Every unclaimed bar is claimable, tier does not matter;
                  ...
                  call to action stays a BUTTON (Roman, 2026-09-15, after a
                  one-day detour as a text line), the last in the column. A
                  claimed bar shows an owner sign-in button in the same place
                  and style instead (Roman, task 122); the ownership-change
                  contact lives on the dashboard, not here. */}
              {!bar.owner_id ? (
